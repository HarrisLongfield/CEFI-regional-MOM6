...
How to use:
python ssh_eval2.py /archive/acr/fre/NWA/2023_04/NWA12_COBALT_2023_04_kpo4-coastatten-physics/gfdl.ncrc5-intel22-prod
"""
...
def plot_ssh_eval(pp_root, label):
    ...
    (model_ssh_index - satellite_ssh_index).plot(ax=ax, c='k', label = f'Model - Altimetry rolling mean')
    ax.set_title('(d) Model index - Satellite index')
    ax.set_xlabel('')
    ax.set_ylabel('Difference in Index (positive: model further north)')
    ax.set_ylim(-3,3)
    ax.axhline(y = 0, color = "red")
    ax.legend(ncol=4, loc='lower right', frameon=False, fontsize=8)

    ax = fig.add_subplot(gs[1, :])
    (model_rolled - satellite_rolled).plot(ax=ax, c='k', label = f'Model - Altimetry rolling mean')
    ax.set_title('(d) Model rolled - Satellite rolled')
    ax.set_xlabel('')
    ax.set_ylabel('Difference in Index (positive north)')
    ax.set_ylim(-3,3)
    ax.axhline(y = 0, color = "red")
    ax.legend(ncol=4, loc='lower right', frameon=False, fontsize=8)

    save_figure('gulfstream_eval2.1', label=label, pdf=True)
    ...
    save_figure('mean_ssh_eval2', label=label)
