...
def createccplots1(): 
    ...
    ax1.set_title('Correlation Coefficient of mean GOM SST and GSI', fontsize='16', pad='20') 
    ...
    ax1.annotate(f'p-value: {float(str(p_valuelowgom)[:3])}', (.7, -.85), bbox=dict(boxstyle="square,pad=0.3", fc="lightcoral", ec="brown", lw=2))
    ax1.annotate(f'p-value: {float(str(p_valuemedgom)[:3])}', (1.7, -.5), bbox=dict(boxstyle="square,pad=0.3", fc="lightblue", ec="steelblue", lw=2))
    plt.savefig('GOM_Correlation_Coefficients.png', dpi=300, bbox_inches='tight')

def createccplots2(): 
    fig2, ax2 = plt.subplots() 
    ax2.set_title('Correlation Coefficient of mean NEUS SST and GSI', fontsize='16', pad='20') 
